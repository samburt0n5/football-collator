...
async function getFixturesForNDays(startDate, numberOfDays) {
  ...
  await getFixturesForDate(previousDay).then(fixtureList => {
    fixtureList.forEach(item => {
      fixtures.push(item);
    });
  });
  for (let i = 0; i < numberOfDays; i++) {
    ...
    await getFixturesForDate(nextDay).then(fixtureList => {
      fixtureList.forEach(item => {
        fixtures.push(item);
      });
    });
...
exports.handler = async (event, context) => {
  ...
  if (updateApi) {
    await sendFixturesData(fixtures);
  }
};
...
function extractGoals(actions) {
  return actions.flatMap(action =>
    action.actions.filter(nestedAction => nestedAction.type === 'Goal' ||nestedAction.type === 'Penalty' || nestedAction.type === 'Own Goal')
      .map(goal => ({ id: uuidv4(), playerName: action.playerName, clockTime: goal.timeLabel.value })));
}
